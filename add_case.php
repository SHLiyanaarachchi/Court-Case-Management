...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation
    if (empty($case_id) || empty($case_title) || empty($case_type) || empty($plaintiff_name) ||
        empty($defendant_name) || empty($filing_date) || empty($court_name) || empty($case_status)) {
        $error = 'Please fill in all required fields';
    } else {
        // Check if case ID already exists
        $check_stmt = $conn->prepare("SELECT case_id FROM cases WHERE case_id = ?");
        $check_stmt->bind_param("s", $case_id);
        $check_stmt->execute();
        ...
        if ($check_result->num_rows > 0) {
            ...
            // Insert new case
            $stmt = $conn->prepare("INSERT INTO cases (case_id, case_title, case_type, plaintiff_name, defendant_name, filing_date, court_name, judge_name, case_status, case_description, next_hearing_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

            $next_hearing_date_value = !empty($next_hearing_date) ? $next_hearing_date : null;

            $stmt->bind_param("sssssssssss", $case_id, $case_title, $case_type, $plaintiff_name, $defendant_name, $filing_date, $court_name, $judge_name, $case_status, $case_description, $next_hearing_date_value);
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="case_id" name="case_id" required
                               value="<?php echo htmlspecialchars($_POST['case_id'] ?? ''); ?>"
                    ...
                    <div class="form-group">
                        ...
                        <select id="case_type" name="case_type" required>
                            ...
                            <option value="Commercial" <?php echo (isset($_POST['case_type']) && $_POST['case_type'] === 'Commercial') ? 'selected' : ''; ?>>Commercial</option>
